...
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
COMMENT = 'Event-native development gateway router - JavaScript implementation with full capabilities'
AS
$$
  const SF = snowflake;
  ...
  function generateUUID() {
    const rs = executeSql("SELECT UUID_STRING() as uuid");
    rs.next();
  ...
  function getStageMD5(stageUrl) {
    try {
      const rs = executeSql(`SELECT METADATA$FILENAME, METADATA$FILE_CONTENT_KEY as md5 
                             FROM @${stageUrl.substring(1)} 
  ...
  function checkVersionGate(objectName, objectType, expectedVersion) {
    ...
    try {
      ...
      if (rs.next()) {
        const currentVersion = rs.getColumnValue('VERSION');
  ...
  try {
    ...
    switch (ACTION) {
      case 'claim':
        ...
        const claimLog = logEvent('dev.claim', claimPayload);
        result = {
          result: 'ok',
      ...
      case 'release':
        ...
        const releaseLog = logEvent('dev.release', {
          lease_id: PARAMS.lease_id,
      ...
      case 'deploy':
        ...
        // Check version gate
        const versionCheck = checkVersionGate(PARAMS.name, PARAMS.type, PARAMS.expected_version);
      ...
      case 'deploy_from_stage':
        ...
        // Check version gate
        const stageVersionCheck = checkVersionGate(PARAMS.name, PARAMS.type, PARAMS.expected_version);
        ...
        if (PARAMS.expected_md5) {
          const actualMD5 = getStageMD5(PARAMS.stage_url);
        ...
        try {
          ...
          const stageVersion = new Date().toISOString();
      ...
      case 'discover':
        ...
        let discoverSQL = "SELECT * FROM MCP.VW_LATEST_SCHEMA";
        const binds = [];
        
        if (PARAMS.filter) {
          const allowedFilters = ['object_type', 'schema_name', 'database_name'];
          if (allowedFilters.includes(PARAMS.filter.column)) {
        ...
        try {
          const rs = executeSql(discoverSQL, binds);
          let rowCount = 0;
          const objects = [];
      ...
      case 'validate':
        ...
        const ddlToValidate = PARAMS.ddl || '';
        const upperDDL = ddlToValidate.toUpperCase();
        
        const forbidden = ['TRUNCATE', 'DROP TABLE', 'DROP DATABASE', 'ALTER ACCOUNT'];
        const violations = forbidden.filter(pattern => upperDDL.includes(pattern));

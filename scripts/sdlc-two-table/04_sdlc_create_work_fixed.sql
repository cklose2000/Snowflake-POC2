-- ============================================================================
-- 04_sdlc_create_work_fixed.sql
-- FIXED: SDLC_CREATE_WORK procedure with proper INSERT syntax
-- Addresses critical INSERT statement and JavaScript binding issues
-- ============================================================================
...
-- ============================================================================
-- FIXED: Create work item procedure with proper INSERT syntax
-- ============================================================================
...
try {
  ...
  const numberInsertStmt = SF.createStatement({
    sqlText: insertSQL,
    binds: [JSON.stringify(numberPayload), 'SDLC']
  });
  numberInsertStmt.execute();
  
  // Get the event ID for optimistic concurrency
  // Wait a moment for Dynamic Table to process
  const eventSQL = `
    SELECT event_id
    FROM CLAUDE_BI.ACTIVITY.EVENTS
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // Brief wait for Dynamic Table processing
...
-- Grant permissions
-- ============================================================================
GRANT USAGE ON SEQUENCE SDLC_TICKET_SEQ TO ROLE MCP_USER_ROLE;
GRANT USAGE ON SEQUENCE SDLC_TICKET_SEQ TO ROLE MCP_AGENT_ROLE;
...
-- ============================================================================
-- Verification: Test the fixed procedure
-- ============================================================================
